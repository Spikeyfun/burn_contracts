module forgelp::target_order_usdc {
    ...
    const MODULE_RESOURCE_ACCOUNT_SEED: vector<u8> = b"forgelp_resource_account_usdc";
    const BURN_ADDRESS: address = @BURN_ADDR;
    const SUPUSDC_ADDRESS: address = @supUSDC_ADDR;
    ...
    public entry fun forge_lp_and_burn<CoinType>(
        ...
        let usdc_metadata: Object<Metadata> = object::address_to_object(SUPUSDC_ADDRESS);
        if (!primary_fungible_store::primary_store_exists(resource_addr, usdc_metadata)) {
            primary_fungible_store::create_primary_store(resource_addr, usdc_metadata);
        };

        primary_fungible_store::transfer(sender, usdc_metadata, resource_addr, amount_usdc_desired);

        let cointype_metadata = coin_wrapper::get_wrapper<CoinType>();
        let pair = amm_pair::liquidity_pool(cointype_metadata, usdc_metadata);
        let (reserve0, reserve1, _) = amm_pair::get_reserves(pair);

        let (reserve_cointype, reserve_usdc) = if (sort::is_sorted_two(cointype_metadata, usdc_metadata)) {
            (reserve0, reserve1)
        } else {
            (reserve1, reserve0)
        };

        if (reserve_usdc == 0 || reserve_cointype == 0) {
            abort(ERR_INSUFFICIENT_LIQUIDITY)
        };
        assert!(reserve_usdc >= 10000, error::invalid_argument(ERR_INSUFFICIENT_LIQUIDITY));
        ...
        let amount_cointype_needed = ((((amount_usdc_desired as u128) * (reserve_cointype as u128)) / (reserve_usdc as u128)) as u64);
        ...
        event::emit(LpBurnedEvent {
            sender: signer::address_of(sender),
            coin_type: type_info::type_name<CoinType>(),
            amount_usdc: amount_usdc_desired,
            timestamp: timestamp::now_seconds(),
        });

        let exp_token_addr = token_manager::get_experience_token_address();
        let (price, price_dec) = price_aggregator::get_coin_price_in_usd<SupraCoin>();
        assert!(price > 0, error::invalid_state(ERR_ZERO_PRICE));

        let usdc_dec = fungible_asset::decimals(usdc_metadata);
        let supra_metadata = coin_wrapper::get_wrapper<SupraCoin>();
        let supra_dec = fungible_asset::decimals(supra_metadata);

        let ten_pow_usdc = pow10(usdc_dec);
        let ten_pow_supra = pow10(supra_dec);
        let price_dec_u8 = (price_dec as u8);
        assert!(price_dec_u8 <= 255, error::invalid_argument(ERR_PRICE_DECIMALS_TOO_LARGE));
        let ten_pow_price = pow10(price_dec_u8);

        let reward_amount_u128 = ((amount_usdc_desired as u128) * ten_pow_price * ten_pow_supra) / (price * ten_pow_usdc);
    ...
    public entry fun forge_lp_fa_and_burn(
        ...
        let pair = amm_pair::liquidity_pool(fa_metadata, usdc_metadata);
        let (reserve0, reserve1, _) = amm_pair::get_reserves(pair);

        let (reserve_fa, reserve_usdc) = if (sort::is_sorted_two(fa_metadata, usdc_metadata)) {
            (reserve0, reserve1)
        } else {
            (reserve1, reserve0)
        };

        if (reserve_usdc == 0 || reserve_fa == 0) {
            abort(ERR_INSUFFICIENT_LIQUIDITY)
        };
        assert!(reserve_usdc >= 10000, error::invalid_argument(ERR_INSUFFICIENT_LIQUIDITY));
        ...
        let amount_fa_needed = ((((amount_usdc_desired as u128) * (reserve_fa as u128)) / (reserve_usdc as u128)) as u64);
        ...
        let (tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin) = if (sort::is_sorted_two(fa_metadata, usdc_metadata)) {
            (fa_address_sorted, SUPUSDC_ADDRESS, amount_fa_needed, amount_usdc_desired, amount_fa_min, amount_usdc_min)
        } else {
            (SUPUSDC_ADDRESS, fa_address_sorted, amount_usdc_desired, amount_fa_needed, amount_usdc_min, amount_fa_min)
        };
    ...
    #[view]
    public fun calculate_expected_reward(amount_usdc_desired: u64): u64 {
